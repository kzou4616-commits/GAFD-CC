...
class GADF:
    ...
    def set_state(self, train_mean, global_mean_logit_score, class_mean_logit_score):
        self.train_mean = train_mean.to(self.device)
        self.global_mean_logit_score = global_mean_logit_score.to(self.device)
        self.class_mean_logit_score = class_mean_logit_score.to(self.device)
    ...
    @torch.no_grad()
    def get_class_mean_logit_score(self,logits):
        class_mean_logit_score = []
        for i in range(len(logits)):
            class_mean_logit_score.append(torch.mean(self.get_logits_score(logits[i])))
        return torch.stack(class_mean_logit_score)
    ...
    def get_state(self,features,logits):
        ...
        class_mean_logit_score = self.get_class_mean_logit_score(logits)
        return train_mean, global_mean_logit_score, class_mean_logit_score
    ...
    def eval(self, data_loader):
        ...
        for (images, _) in tqdm(data_loader):
            ...
            class_ids = torch.argmax(torch.softmax(logit, dim=1), dim=1).to(self.device)
            tm = self.train_mean[class_ids]
            dist = feature - tm
            sg_all = torch.sign(w)

            ep_dist_all = dist[:, None, :] * sg_all[None, :, :]
            ep_dist_all[ep_dist_all < 0] = 0
            ep_dist_combined = torch.mean(ep_dist_all, dim=1)
            ep_error = ep_dist_combined.norm(dim=1, p=1) / feature.norm(dim=1, p=1)

    
            # 同样处理负方向的误差
            en_dist_all = dist[:, None, :] * (-sg_all[None, :, :])
            en_dist_all[en_dist_all < 0] = 0
            en_dist_combined = torch.mean(en_dist_all, dim=1)
            en_error = en_dist_combined.norm(dim=1, p=1) / feature.norm(dim=1, p=1)

    
            logit_score = self.get_logits_score(logit)
            score =  ep_error/(logit_score + self.class_mean_logit_score[class_ids]) +  en_error/(self.global_mean_logit_score + self.class_mean_logit_score[class_ids])
